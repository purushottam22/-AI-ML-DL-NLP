...
# hisory = model.fit(trainx, trainy, batch_size=32, verbose=1, epoch=10, callbacks = [])

"""Model with custom loss changes in between epochs"""


class StoreEpochs(tf.keras.callbacks.Callback):
    def __init__(self, epoch: int = 0):
        super().__init__()
        self.my_epoch = tf.keras.backend.variable(epoch)

    def on_epoch_begin(self, epoch, logs=None):
        tf.keras.backend.set_value(self.my_epoch, epoch)


def alpha_update(epoch):
    if epoch < 10:
        return 1.0 - 0.1 * epoch
    return 0.0
